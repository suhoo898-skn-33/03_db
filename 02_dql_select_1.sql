...
# - 특정 col을 기준으로 삼아서 오름차순(ASC) / 내림차순(DESC) 지정
# - 기준으로 삼은 col이 여러 개 존재하면, 그룹화된 정렬이 수행된다.
...
from
    tbl_menu
where
    # category_code = 4 or category_code = 6 or category code = 10
    # not in
...
where
    ...
    # -> null은 값(data)가 아니라 컬럼이 비었음을 나타내는 기호
    # ref_category_code = null;   # 모든 행 false

    # ref_category_code 컬럼값이 비어있으면 True
